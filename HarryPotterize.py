...
if __name__ == "__main__":
    ...
    # 1) Read images
    cv_cover = cv2.imread('../data/cv_cover.jpg')   # The textbook cover
    cv_desk = cv2.imread('../data/cv_desk.png')     # The desk scene (book on desk)
    hp_cover = cv2.imread('../data/hp_cover.jpg')   # The Harry Potter cover

    # 2) Match features between the cover and the desk
    #    locs1 => keypoints in cv_cover
    #    locs2 => keypoints in cv_desk
    ...
    # 4) Compute the robust homography using RANSAC
    #    Our function computeH_ransac expects x1 as "template" points
    #    and x2 as "image" points, returning H: x1 ≈ H * x2
    H, inliers = computeH_ransac(x1, x2, nSamples=20, threshold=10)

    # 5) Resize hp_cover to the same shape as cv_cover
    #    so that it fully replaces the textbook area
    ...
    # 7) (Optional) Cross-check with OpenCV’s findHomography
    if DISPLAY:
        # OpenCV wants points as float32 and in shape (N,1,2)
