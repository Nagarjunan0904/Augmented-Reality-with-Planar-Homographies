...
if __name__ == "__main__":
    ...
    # Histogram of match counts for each rotation
    nMatches = []
    ...
    for angle in angles:
        # 1) Rotate Image
        # Using reshape=False keeps the original image size (with black borders)
    ...
    plt.tight_layout()
    plt.bar(x=angles, height=nMatches, width=5, color='blue')
    plt.xlabel('Rotation Angle (degrees)')
    plt.ylabel('Number of Matches')
    plt.title('BRIEF - Number of Matches vs. Rotation Angle')
    plt.show()
